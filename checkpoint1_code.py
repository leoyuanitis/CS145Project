...
# warning filters are implemented w/ the help from LLM, but there's still some warnings left, so expect warning messages;
...
class KNNRecommender(BaseRecommender):
    ...
    def fit(self, log, user_features=None, item_features=None):
        ...
        #self.model.fit(x, y)
        # implement cross validation to ensure good hyperparam 
        pipe = Pipeline([
    ...
    def predict(self, log, k, users, items, user_features=None, item_features=None, filter_seen_items=True):
        ...
        #alpha = 0.1
        #cross['relevance'] = cross["prob"] * (1 + alpha * np.log1p(cross["price"]))
        #cross["rank"] = cross.groupby("user_idx").cumcount()
        #cross["position_decay"] = 1 / np.log2(2 + cross["rank"])
        # involve ranking hurts the discoutned revenue metrics!!!
...
class RFRecommender(BaseRecommender):
    ...
    def fit(self, log, user_features=None, item_features=None):
        if user_features and item_features:
            ...
            # implement hyper parameter tuning and feature importance analysis
            param_grid = {
                'n_estimators': [50, 100],   
            ...
            gridSearch = GridSearchCV(
                rf, param_grid, cv=5, scoring='roc_auc', n_jobs=-1, verbose=False
            )
            gridSearch.fit(x, y)

            self.model = gridSearch.best_estimator_

            print("Best RF params:", gridSearch.best_params_)
            print("Best RF CV score:", gridSearch.best_score_)
    ...
    def predict(self, log, k, users, items, user_features=None, item_features=None, filter_seen_items=True):
        ...
        # Get the probability for the "relevant" class
...
def run_recommender_analysis():
    ...
    recommenders = [
        ...
        RFRecommender(seed=42),
        GBRecommender(seed=42)
    ]
    # recommender_names = ["SVM", "Random", "Popularity", "MyRecommender"]
    recommender_names = ["SVM", "Random", "Popularity", "ContentBased", "KNN", "RF", "GB"]
